MIdiagRDD<-function(
	y, x, cut, seed=1, M1=100, M2=5, p2s1=1, emp=0, bw="mserd",
...
x1<-df1$x; y3<-df1$y; cut<-cut; q1<-p1+1
...
#############
#Bandwidth Selection
rbd1<-rdbwselect(data3$y3,data3$x1,c=cut,bwselect=bw,kernel=ker,p=p1,q=q1,covs=covs1)
h1<-rbd1$bws[1]*bwidth
h2<-rbd1$bws[2]*bwidth
data3a<-data3[(cut-h1<=(data3$x1))&((data3$x1)<=cut+h2),]
rddn<-nrow(data3a)

rbdMI1<-rdbwselect(data3$y3,data3$x1,c=cut,bwselect=bw,kernel=ker,p=p1,q=q1,covs=covs1)
hMI1<-rbdMI1$bws[1]*bwidth
hMI2<-rbdMI1$bws[2]*bwidth
...
for(i in 1:M1){
 ...
 tauhat0[,i]<-a.out$imputations[[i]]$y0
 tauhat1[,i]<-a.out$imputations[[i]]$y1
...
modelRDD1<-rdrobust(data3$y3, data3$x1, c=cut,all=TRUE,h=c(h1,h2),kernel=ker,p=p1,q=q1)
...
output<-data.frame(rownames,Estimate,Std.Error,CI.LL,CI.UL,size,ratio)
output
